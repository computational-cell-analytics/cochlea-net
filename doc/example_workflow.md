...
Pass an IHC segmentation as a fourth argument to the first step, and as the second argument to the
third step. It is dilated into the mask that restricts the prediction to the region around the
IHCs, which is the standard case: on a full cochlea it skips about 97 % of the blocks. The third
step then also writes `synapse_detection_filtered.tsv`, with the detections matched to the IHCs.
Without a segmentation the prediction falls back to the full volume, and the first step warns
about it.
