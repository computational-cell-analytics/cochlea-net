"""Edit OTOF positive and negative IHC assignments in napari."""
...
def mask_signal_to_ihcs(signal: np.ndarray, ihcs: np.ndarray) -> np.ndarray:
    """Set the signal outside the IHC mask to zero in place."""
...
def load_editor_data(cochlea: str, scale: int) -> EditorData:
    ...
    if missing_ids:
        warnings.warn(
            f"{len(missing_ids)} assigned IHC IDs are absent at {scale_key}. "
            "They will remain unchanged in the exported table."
        )

    scale_zyx = tuple(value * (2 ** scale) for value in BASE_VOXEL_SIZE_XYZ[::-1])
    return EditorData(table=table, ihcs=ihcs, marker_mask=marker_mask, otof=otof, scale_zyx=scale_zyx)
...
def run_editor(cochlea: str, output_table: str, data: EditorData) -> None:
    ...
    viewer = napari.Viewer(title=f"OTOF assignment editor | {cochlea}")
    viewer.add_image(
        data.otof,
        name=OTOF_NAME,
        colormap="gray",
...
def main() -> None:
    ...
    data = load_editor_data(args.cochlea, args.scale)
