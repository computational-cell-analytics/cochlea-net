...
def threshold_marker(
    ...
    for cochlea in cochleae:
        ...
        if n_missing > 0:
            print(f"{cochlea}: {n_missing} of {len(valid_ids)} instances have no object measure and stay unassigned.")
        ...
        n_positive, n_negative = int(is_positive.sum()), int((~is_positive).sum())
        ...
        if threshold_save_dir is not None:
            ...
            param_dict = {
                ...
                "percent_passing_each_threshold": per_feature,
            }
            export_dictionary_as_json(param_dict, os.path.join(threshold_save_dir, f"{name}.json"),
                                      force_overwrite=True)

        os.makedirs(plot_dir, exist_ok=True)
        _plot_thresholds(features, thresholds, is_positive, name, method,
