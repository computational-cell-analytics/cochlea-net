...
image_dir = os.path.join(cochlea_dir, "predictions/val_ihc/images2D")
image_dir = os.path.join(cochlea_dir, "AnnotatedImageCrops/F1ValidationIHCs")
out_dir = os.path.join(cochlea_dir, "predictions", "val_ihc", "micro-sam_finetuned")
model_dir = os.path.join(cochlea_dir, "trained_models", "IHC")

images = [entry.path for entry in os.scandir(image_dir) if entry.is_file()]
checkpoint = os.path.join(model_dir, "cochlea_micro_sam_2026-07-28_micro-sam_ihc-v11_2d.pt")
